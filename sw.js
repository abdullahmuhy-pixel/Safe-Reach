...
// self.scriptURL is the URL of *this* (new) SW file — reliable during install,
// unlike self.registration.active?.scriptURL which points to the OLD worker.
async function computeSWHash() {
  try {
    const url  = self.scriptURL || './sw.js';
...
self.addEventListener('install', event => {
  ...
  const SHELL_URLS = [
    ...
    './index.html',
    './manifest.json',
    // Icons must be here so they are pre-cached AND refreshed whenever
    // the content hash changes (new cache name = fresh network fetch).
    './icon-96.png',
    './icon-192.png',
    './icon-512.png',
...
function isAppShell(url, request) {
  return (
    ...
    url.pathname === '/'           ||
    // App icons — cache-first so they work offline; refreshed on every SW update
